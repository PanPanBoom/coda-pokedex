...
export const PokemonStats = (props: PokemonStatsProps) => {
    return (
        <article className={props.className + " p-2 border-x-4 border-t-4 border-yellow-100 flex-1 flex flex-col"}>
            ...
            <div className="text-4xl flex place-content-between items-center my-auto">
                ...
                <ul className="flex-initial w-32">
                    {props.stats.map((stat, index) => <li className="my-1 bg-yellow-100" key={index}>{stat.base_stat}</li>)}
                </ul>
            </div>
        </article>
    )
